...
<div class="tbar">
    ...
    <span style="display:inline-flex;gap:3px;">
        <a href="<?= url('soportes', ['caja_id' => $cajaId, 'orientacion' => 'horizontal']) ?>" class="btn-c <?= $orientacion==='horizontal'?'act':'' ?>"><i class="bi bi-arrows-angle-expand"></i> Horizontal</a>
        <a href="<?= url('soportes', ['caja_id' => $cajaId, 'orientacion' => 'vertical']) ?>" class="btn-c <?= $orientacion==='vertical'?'act':'' ?>"><i class="bi bi-arrows-angle-contract"></i> Vertical</a>
    </span>
    <div class="sep"></div>
    <label>Zoom</label>
...
?>
    <div class="cpage" data-page="<?= $numPag ?>">
        <div class="phdr">
            <?php if ($tieneLogo): ?>
                <div class="phdr-logo"><img src="<?= asset('uploads/logo.png') ?>?t=<?= time() ?>" alt="Logo"></div>
            <?php endif; ?>
            <div class="phdr-text">
        ...
        <?php foreach ($chunk as $s):
            $imgSrc = ($s['archivo'] && strpos($s['archivo'], 'uploads/') === 0) ? htmlspecialchars(asset($s['archivo'])) : '';
        ?>
