...
// Calcular tamaño de fuente según el contenido más largo
$maxChars = 0;
...
// Paginación: max 24 gastos por hoja para evitar desborde al imprimir
$maxPerPage = 24;
if (empty($gastosData)) {
...
$notaGuardada = '';
if ($cajaId) {
    $stmtN = $pdo->prepare('SELECT nota FROM cajas WHERE id = ?');
    $stmtN->execute([$cajaId]);
    $rowN = $stmtN->fetch();
    $notaGuardada = $rowN ? $rowN['nota'] : '';
}
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $cajaId) {
    if (isset($_POST['guardar_nota'])) {
        $notaGuardada = $_POST['nota_caja'] ?? '';
        $stmtU = $pdo->prepare('UPDATE cajas SET nota = ? WHERE id = ?');
        $stmtU->execute([$notaGuardada, $cajaId]);
    } elseif (isset($_POST['eliminar_nota'])) {
        $notaGuardada = '';
        $stmtU = $pdo->prepare('UPDATE cajas SET nota = NULL WHERE id = ?');
        $stmtU->execute([$cajaId]);
    }
}
?>
<!doctype html>
